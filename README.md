...
| <a name="input_public_ips_for_slaves"></a> [public\_ips\_for\_slaves](#input\_public\_ips\_for\_slaves) | Flag for weather or not to assign public IP's to slaves. This will give each slave a public IP address to make it look like it's coming from a different source instead of routing traffic out of one of the 3 NAT gateways | `bool` | `false` | no |
